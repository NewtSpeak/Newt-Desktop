...
//! 移动端无 Discord IPC，仅提供空实现供 command 注册。
...
pub fn get_rpc_activity() -> Option<RpcActivity> {
  ...
  {
    desktop::get_rpc_activity_inner()
  }
}

/// 启动后台 IPC 监听（幂等，可在 setup 中调用）。仅桌面导出。
#[cfg(not(any(target_os = "android", target_os = "ios")))]
pub fn start_discord_rpc_server() {
  desktop::start_discord_rpc_server()
}
...
mod desktop {
  ...
  pub(super) fn get_rpc_activity_inner() -> Option<RpcActivity> {
    RPC_STATE.lock().ok().and_then(|g| g.clone())
  }

  pub(super) fn start_discord_rpc_server() {
  ...
  fn serve_once() -> Result<(), String> {
    ...
    for i in 0..10 {
      ...
      paths.push(format!(
        "{runtime}/app/com.discordapp.Discord/discord-ipc-{i}"
      ));
    }
